...
import numpy as np
from typing import Any, List, Mapping, Optional, OrderedDict, Tuple, Dict
import torch
...
from dawsod.layers.gradient_scalar_layer import GradientScalarLayer
...
class Protype(nn.Module):
    def __init__(self,
        ...
        self.value = nn.Parameter(
            torch.randn(size,device=device,requires_grad=requires_grad),
            requires_grad=requires_grad)


@META_ARCH_REGISTRY.register()
class DAWSOD_RCNN(GeneralizedRCNN):
    """
    ...
    def __init__(
            ...
            *,
            ...
            cam_heads: nn.Module,
            # fcos_heads: nn.Module,
            pixel_mean: Tuple[float],
            pixel_std: Tuple[float],
            input_format: Optional[str] = None,
            vis_period: int = 0,
            protype: Optional[nn.Module] = None,
    ):
        """
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            # "fcos_heads": FCOSHead(cfg,[backbone.output_shape()[cfg.MODEL.ROI_HEADS.IN_FEATURES[0]]]),
            "input_format": cfg.INPUT.FORMAT,
            "vis_period": cfg.VIS_PERIOD,
            "pixel_mean": cfg.MODEL.PIXEL_MEAN,
            "pixel_std": cfg.MODEL.PIXEL_STD,
            "protype": Protype(size=(cfg.MODEL.ROI_HEADS.NUM_CLASSES,cfg.MODEL.ROI_BOX_HEAD.FC_DIM),requires_grad=False,device=torch.device(cfg.MODEL.DEVICE))
        }
    ...
    def forward(self, batched_inputs,iter=20000):
        """
        Args:
        ...
        # pixel-level class-wise alignment
        # ctr_losses = self.fcos_heads([source_features[self.roi_heads.in_features[0]]],source_gt_instances)
        # ctr_losses_weak = self.fcos_heads.forward_weak([target_features[self.roi_heads.in_features[0]]],target_gt_instances)
        ...
        # )
        if iter >= 0:
            # img_cls_s_losses = self.roi_heads.forward_weak_pro(
            #     "s",source_features, source_proposals, self.protype.value, source_gt_instances
            # )
            img_cls_t_losses = self.roi_heads.forward_weak_pro(
                "t",target_features, target_proposals, self.protype.value, target_gt_instances
            )
            # losses.update(img_cls_s_losses)
            losses.update(img_cls_t_losses)
        

        ema_rate = self.ema_rate
        # ema_rate = self.exponential_decay(ema_rate,iter)

        with torch.no_grad():
            target_features = self.roi_heads._update_protype(source_features,source_gt_instances,self.protype.value,ema_rate)
        ...
        # losses.update(pc_losses)
        # losses.update(pc_losses_weak)
        return losses
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if detected_instances is None:
            ...
            results = self.roi_heads._forward_box(features, proposals, self.protype.value)
        else:
            detected_instances = [x.to(self.device) for x in detected_instances]
            results = self.roi_heads.forward_with_given_boxes(features, detected_instances)

        if do_postprocess:
            return DAWSOD_RCNN._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
    ...
    @classmethod
    def exponential_decay(self,ema_rate,iters):
        decay_rate = 0.9
        decay_steps = 1000
        bottom = 0.1
        decayed_ema_rate = ema_rate * (decay_rate**(iters / decay_steps) + bottom)
        return decayed_ema_rate
...
class PAHead(nn.Module):
    """
    pixel-level class-wise feature alignment
    """
    ...
    def __init__(self, backbone_out_shape: Dict[str, ShapeSpec] ,num_classes: int):
        ...
        self.pa_conv1 = nn.Conv2d(in_channels, 512, kernel_size=3, stride=2, padding=1, bias=False)
        self.pa_conv2 = nn.Conv2d(512, 128, kernel_size=3, stride=2, padding=1, bias=False)
        self.cls_conv = nn.Conv2d(128, num_classes, kernel_size=3, stride=2, padding=1, bias=False)
        self.ctr_conv = nn.Conv2d(128, 1, kernel_size=3, stride=2, padding=1, bias=False)
        self.pa_bn1 = nn.BatchNorm2d(512)
        self.pa_bn2 = nn.BatchNorm2d(128)
        self._init_weights()
    ...
    def forward(self, x, gt_instances):
        x = F.relu(self.pa_bn1(self.pa_conv1(x)))
        x = F.relu(self.pa_bn2(self.pa_conv2(x)))
        cls_map = self.cls_conv(x)
        ctr_map = self.ctr_conv(x)

        cls_ctr_map = cls_map * ctr_map

        target_ctr = self.compute_ctrness_targets()
