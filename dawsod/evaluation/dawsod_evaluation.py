...
class DomainAdaptiveDetectionEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(self, dataset_name):
        ...
        # use 07 metric following previous works
    ...
    def evaluate(self):
        ...
        res_file_template = os.path.join(
            devkit_path, "results", self._dataset_name, res_file_template
        )
        if not os.path.exists(os.path.join(devkit_path, "results", self._dataset_name)):
            os.makedirs(os.path.join(devkit_path, "results", self._dataset_name))
